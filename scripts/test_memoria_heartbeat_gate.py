...
def scenario_agent_id_comes_from_env() -> None:
    ...
    if expected:
        ...
        # No .env (stripped checkout): the fallback must still be a real local
        # name, never one of the upstream fixtures.
        check_eq("falls back to the local default", gate.AGENT, "hex")
