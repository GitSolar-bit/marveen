...
// newestMainConfigRoot() now LIVES next to mainConfigRoots() in inbound-probe
// (STUCKROOT923 moved it there so every main-agent transcript reader shares one
// home). Two different things are worth pinning, so they are mocked apart:
//   - the SELECTION RULE is exercised on the real function with fixture roots
//     passed in, which is why it takes an injectable `roots` argument;
//   - the WIRING (does configDirFor actually delegate to it) is exercised with
//     the module stubbed, since a same-module call cannot be intercepted.
let roots: string[] = [SHARED_CONFIG, ISOLATED_CONFIG]
let stubbedNewestRoot: string | undefined
vi.mock('../web/inbound-probe.js', () => ({
  mainConfigRoots: () => roots,
  newestMainConfigRoot: () => stubbedNewestRoot,
}))
...
describe('context-restart gate: which config root the main agent is read from', () => {
  ...
  it('the MAIN agent asks for the newest root instead of the host default (the wiring)', async () => {
    // The whole defect was that this returned undefined for the main agent and
    // silently read ~/.claude. Pin the delegation, not the selection.
    const { configDirFor } = await import('../web/context-restart-gate-runner.js')
    stubbedNewestRoot = ISOLATED_CONFIG
    expect(configDirFor('marveen')).toBe(ISOLATED_CONFIG)
    stubbedNewestRoot = SHARED_CONFIG
    expect(configDirFor('marveen')).toBe(SHARED_CONFIG)
    stubbedNewestRoot = undefined
  })

  it('picks the isolated root when its transcript is the newer one', async () => {
    const { newestMainConfigRoot } = await vi.importActual<
      typeof import('../web/inbound-probe.js')>('../web/inbound-probe.js')
    expect(newestMainConfigRoot([SHARED_CONFIG, ISOLATED_CONFIG])).toBe(ISOLATED_CONFIG)
  })
  ...
  it('picks the shared root when THAT is the newer one (pre-migration history is real)', async () => {
    ...
    expect(newestMainConfigRoot([SHARED_CONFIG, ISOLATED_CONFIG])).toBe(SHARED_CONFIG)
  ...
  it('returns undefined when no candidate root holds a transcript, so the caller default still applies', async () => {
    ...
    roots = [EMPTY_CONFIG]
    try {
      expect(newestMainConfigRoot([EMPTY_CONFIG])).toBeUndefined()
    } finally {
      roots = saved
    }
  })
